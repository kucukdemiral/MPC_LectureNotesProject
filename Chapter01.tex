...
\begin{figure}[H]
	...
	\begin{tikzpicture}[auto, node distance=2.5cm, >=latex', thick]
		...
		\tikzset{block/.style={draw, rectangle, fill=gray!20, minimum height=3em, minimum width=5em}}
...
As shown in Figure~\ref{fig:mpc_loop}, MPC takes a different approach. In an MPC architecture, the explicit controller block is replaced by an \emph{optimisation routine}. Here we call it \emph{Optimiser}. This is typically an embedded digital computer that runs an algorithm.  

Instead of specifying \emph{how} to calculate the input mathematically, MPC specifies \emph{what} the controller should achieve. That is, no explicit controller structure is specified; instead, the optimiser directly generates the control signal. Even if the error signal $e(t)$ is known, we cannot predict what the controller will generate. The signal generated by the optimiser that goes into the \emph{Plant} depends on many factors and conditions of the controller,  measured signal,  dynamics of the plant, reference signal, environmental conditions, and their current, past and predicted values. 
...
\begin{figure}[t]
	...
	\begin{tikzpicture}[auto, node distance=2.5cm, >={Latex[length=3mm, width=2mm]}, thick]
		...
		% 1. Define Styles
		% ---------------------------------------------------------
		\tikzset{block/.style={draw, rectangle, fill=gray!30, minimum height=4em, minimum width=6em, text width=6em, align=center}}
		\tikzset{line/.style={draw, ->}}
...
In MPC, this is known as a \emph{receding horizon} strategy. The controller calculates an optimal sequence of actions for the entire horizon but only executes the immediate first step. A fraction of a second later, it takes a new measurement and repeats the entire optimisation. This constant re-planning makes the system robust to uncertainty and ensures the car remains on the track, even in a changing or unpredictable environment.

\section{Receding Horizon Control (RHC)}

The mathematical framework that brings the racing car analogy to life is known as \textit{Receding Horizon Control} (RHC). This strategy is the mechanism that connects static optimisation with dynamic control.
...
\begin{enumerate}
	...
	\item \textbf{Execute:} Although a full sequence of $N$ steps was calculated, the controller applies \emph{only the first element}, $u_0$, to the plant. The remaining $N-1$ steps of the plan are discarded.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[auto, >={Latex[length=3mm, width=2mm]}, thick]
		...
		% 1. Define Styles
		\tikzset{block/.style={draw, rectangle, fill=gray!30, minimum height=4em, minimum width=6em, align=center}}
		\tikzset{line/.style={draw, ->}}
		...
		\begin{scope}[yshift=-8cm, xshift=-4cm]
			...
			% Styles for bars
			\tikzset{dobar/.style={draw, fill=yellow!80!red, minimum height=0.8cm}}
			\tikzset{planbar/.style={draw, fill=gray!30, minimum height=0.8cm, anchor=west}}
...
Once the cost function is written out explicitly, its interpretation becomes quite intuitive. The cost $J$ acts like a balance sheet: at every time step, the controller adds up a set of penalties and then chooses the control action that makes this total as small as possible:
\begin{equation}
...
At every specific time step $t$, the controller solves the optimisation problem to find the optimal \emph{sequence} of future control inputs, denoted as ${U}^\star$:
\begin{equation}
	{U}^\star = \{u^\star_{0}, u^\star_{1}, \dots, u^\star_{N-1}\} = \arg \min_{{U}} J(x_0, {U})
\end{equation}
This sequence represents a complete plan for the next $N$ time steps. For example, in a self-driving car, this sequence might represent the steering angles for the next 10 seconds. However,  MPC does \emph{not} implement the entire plan but instead proceeds as follows:
...
\tikzset{cloud/.style={ellipse, draw, fill=red!10,
...
\begin{notebox}{Practical Guideline: When to use MPC?}
	...
	\begin{itemize}
		\item If a system is simple (SISO), linear, and operates far from its physical limits, a classical \emph{PID controller} is computationally cheaper and often sufficient.
		\item MPC should be reserved for problems where its unique ability to handle \emph{constraints}, \emph{multivariable interactions}, and \emph{future look-ahead} justifies the added complexity.
	\end{itemize}
\end{notebox}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item[(d)] Referring to the ``shower analogy'' in Section~\ref{sec:shower_analogy}, describe in plain language how a \emph{decentralised PID} strategy and the \emph{MPC centralised} strategy each attempt to regulate both temperature and pressure. Why does the PID strategy lead to oscillation?
	\end{enumerate}
