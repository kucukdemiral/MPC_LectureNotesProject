...
% Figures are referenced with the same relative paths as the textbook.
...
% Keep figure addresses compatible with the book (slides assumed to live next to them).
  ...
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
    \IfFileExists{#2.pdf}{\includegraphics[#1]{#2.pdf}}{%
      \IfFileExists{#2.png}{\includegraphics[#1]{#2.png}}{%
        \fbox{\parbox[c][0.26\textheight][c]{0.92\linewidth}{\centering\small Figure file not found at\\\texttt{#2}\\(Place the slides next to the book so the \texttt{Figures/} folder is available.)}}%
      }%
    }%
  }%
}
...
\title[LQR via QP]{LQR Design via Quadratic Programming\\\small (Chapter 5 in the MPC textbook)}
...
\begin{figure}
  ...
  \caption{Finite-horizon LQR for the double integrator (figure address preserved from the book).}
\end{figure}
\end{frame}
...
\begin{figure}
  ...
  \caption{Convergence of the Riccati recursion (figure address preserved from the book).}
\end{figure}
\end{frame}
...
\begin{figure}
  ...
  \caption{Infinite-horizon LQR for the double integrator (figure address preserved from the book).}
\end{figure}
\end{frame}
...
\begin{frame}{MagLev schematic (from the book)}
\begin{figure}
\centering
...
\begin{figure}
  ...
  \caption{MagLev setpoint tracking (figure address preserved from the book).}
\end{figure}
\end{frame}
