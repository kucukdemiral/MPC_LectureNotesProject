...
Chapters on Kalman filtering, LQG, and stochastic MPC.
...
probability}.  The four main applications you will encounter in this
book are:
...
The \emph{sample space} $\Omega$ is the set of all possible outcomes
of a random experiment.  An \emph{event} $A$ is any subset
$A \subseteq \Omega$.  The \emph{probability} of event $A$, written
...
The \emph{conditional probability} of event $A$ given that event $B$
...
Events $A$ and $B$ are \emph{independent} if knowing $B$ occurred
...
\]
By the \emph{law of total probability}:
\[
...
\]
In words: the \emph{posterior} probability of $A$ given evidence $B$
equals the \emph{likelihood} $P(B\mid A)$ times the \emph{prior}
$P(A)$, divided by the \emph{marginal likelihood} (evidence) $P(B)$.
\end{defnbox}
...
A \emph{random variable} (RV) $X$ is a function that assigns a
numerical value to every outcome in the sample space $\Omega$.  It
is \emph{discrete} if it takes countably many values and
\emph{continuous} if it takes values in a continuous range.
\end{defnbox}
...
\emph{probability density function} (PDF) $f_X(x)$, defined so that:
\[
...
The \emph{cumulative distribution function} (CDF) collects
probability up to $x$:
\[
...
\emph{Central Limit Theorem}: the sum of many independent random
...
\[
  ...
  \quad x \in \mathbb{R},
\]
where $\mu$ is the \emph{mean} (centre) and $\sigma^2$ is the
\emph{variance} (spread).  The \emph{standard deviation} is
$\sigma = \sqrt{\sigma^2}$.
\end{defnbox}
...
The \emph{expected value} (or \emph{mean}) of a random variable
...
The \emph{variance} measures the average squared deviation from the
mean:
\[
  \sigma^2 = \mathrm{Var}(X) = \mathbb{E}\!\left[(X-\mu)^2\right]
           = \mathbb{E}[X^2] - \mu^2.
\]
The \emph{standard deviation} $\sigma = \sqrt{\mathrm{Var}(X)}$ has
...
The \emph{covariance} of two random variables $X$ and $Y$ measures
...
The \emph{correlation coefficient} normalises covariance to the
range $[-1,1]$:
\[
...
\emph{multivariate Gaussian} distribution with mean vector
$\boldsymbol{\mu} \in \mathbb{R}^n$ and
\emph{covariance matrix} $\Sigma \in \mathbb{R}^{n\times n}$ if its
PDF is:
\[
...
\]
The covariance matrix $\Sigma$ must be \emph{symmetric positive
...
\begin{notebox}{Covariance matrices in the Kalman filter}
  ...
  where $P_k$ is the \emph{error covariance matrix}.  At each step:
  \begin{itemize}[noitemsep]
...
This is exactly the \emph{predict step} of the Kalman filter.  If
...
A \emph{stochastic process} is a sequence of random variables
...
The \emph{state-space model} is:
\begin{align*}
  {x}_{k+1} &= A{x}_k + B{u}_k + {w}_k,
  \qquad {w}_k \sim \mathcal{N}({0}, Q), \\
  {y}_k     &= C{x}_k + {v}_k,
  \qquad {v}_k \sim \mathcal{N}({0}, R).
\end{align*}
...
\emph{Markov Decision Process} (MDP), which is a direct application
...
\begin{itemize}[noitemsep]
  ...
  \item $\mathcal{P}(s' \mid s, a)$: \emph{transition probability} —
