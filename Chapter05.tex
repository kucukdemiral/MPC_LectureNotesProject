...
In this chapter, we adopt a numerical optimisation perspective. Instead of solving the control problem recursively (time-step by time-step), we will formulate the entire trajectory optimisation as a single, large static task—specifically, a \emph{Quadratic Programme (QP)}.
...
\begin{itemize}
	\item The state weighting matrices ($Q_k, Q_N$) must be \emph{Positive Semi-Definite} ($Q \succeq 0$). This ensures the cost function is convex (bowl-shaped), punishing deviations from the origin.
	\item The control weighting matrices ($R_k$) must be \emph{Positive Definite} ($R \succ 0$). This ensures strict convexity with respect to the input, preventing the solver from requesting infinite control energy.
\end{itemize}
...
We stack the control inputs and the resulting states chronologically. Note that the initial state $x_0$ is \emph{not} a decision variable (it is a measured constant), so the vector starts with $u_0$:
\begin{equation*}
...
The system dynamics impose a linear equality constraint at every time step. Rearranging $x_{k+1} = A_k x_k + B_k u_k$ gives:
\begin{equation*}
	A_k x_k + B_k u_k - x_{k+1} = 0, \qquad k = 1, \dots, N{-}1.
\end{equation*}
For the first step ($k=0$), the initial state $x_0$ is known and moves to the right-hand side:
\begin{equation*}
	B_0 u_0 - x_1 = -A_0 x_0
\end{equation*}
...
With the LQR problem cast as an equality-constrained Quadratic Programme, we can determine the exact solution by applying the \emph{Karush-Kuhn-Tucker (KKT) conditions}. 
...
Combining these yields the \emph{KKT System}, a linear matrix equation:
\begin{equation*}
	\begin{bmatrix}
		H & \mathcal{C}^\top \\
		\mathcal{C} & 0
...
While the large matrix above can be inverted directly using a computer (which is how MPC solvers typically work), we can derive the fundamental equations of optimal control by solving it analytically via \emph{block back-substitution}.
...
\textbf{Conclusion:}
This pattern repeats for every step backwards, giving us the famous \emph{Riccati Equation}.
...
In the previous section, we derived the LQR solution for a specific horizon $N$. The resulting controller, $u_k = -K_k x_k$, is \emph{time-varying}; the feedback gain $K_k$ changes at every single time step as the system approaches the final horizon.
...
\end{equation*}
Now, applying the steady-state condition $P_k = P_{k+1} = P$, we arrive at the \emph{Discrete Algebraic Riccati Equation (DARE)}:
\begin{equation*}
...
\begin{examplebox}{Numerical Example: Infinite-Horizon Stabilisation}
	To demonstrate the power and simplicity of the infinite-horizon LQR controller, we revisit the \emph{Double Integrator} problem. This system represents a simple mass controlled by a force, discretised with a sampling time of $h=0.1$ seconds.
	...
	To do this, we solve the \emph{Discrete Algebraic Riccati Equation (DARE)}. In MATLAB, this is automated by the command \texttt{dare}, which returns the steady-state solution $P_{\infty}$ and the optimal gain $K_{\infty}$.
...
\begin{examplebox}{Intuition: The Hovering Drone}
	...
	To satisfy the equilibrium condition ($\dot{v} = 0$), the motors must generate a constant upward thrust exactly equal to the drone's weight ($mg$). Therefore, the equilibrium input is $u_g = mg$. Without this non-zero \emph{feedforward} term, the controller would constantly fight against gravity, resulting in a steady-state error.
\end{examplebox}
...
\end{equation*}
This reveals that the \emph{error dynamics are identical to the original system dynamics}. Consequently, we can design a standard LQR gain $K$ for the error system to drive $\tilde{x} \to 0$.
