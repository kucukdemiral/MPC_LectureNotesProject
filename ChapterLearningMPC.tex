...
\begin{notebox}{Connection to Earlier Chapters}
	Each approach builds on tools already developed in this book:
	\begin{center}
		\small
		\begin{tabular}{@{}l p{0.62\linewidth}@{}}
			...
			\midrule
			GP-MPC & Ch.\,6 (Bayesian estimation), Ch.\,7 (constrained MPC), Tube MPC \\
...
\begin{align*}
	\mu_*(x_*) &= k_*^\top \bigl(K + \sigma_n^2 I\bigr)^{-1} y \\[4pt]
	\sigma_*^2(x_*) &= k(x_*, x_*) - k_*^\top \bigl(K + \sigma_n^2 I\bigr)^{-1} k_*
\end{align*}
where:
\begin{itemize}[noitemsep]
...
\begin{equation}\label{eq:tightened_chance}
	{\mu_{k,i} + \Phi^{-1}(1-\delta)\;\sigma_{k,i} \le x_{\max,i}}
\end{equation}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[
		...
		blockplant/.style={draw, rounded corners=3pt, fill=green!10,
			minimum width=2.8cm, minimum height=1.0cm, align=center, font=\small},
		sum/.style={draw, circle, minimum size=0.6cm, inner sep=0pt, font=\footnotesize},
		arr/.style={->, thick},
	]
		% ---- Nodes ----
		\node[block] (mpc) at (0,0) {MPC Optimiser\\{\scriptsize (NLP solver)}};
		\node[blockplant] (plant) at (6,0) {True Plant\\$x_{k+1} = f(x_k, u_k)$};
		\node[blockgp] (gp) at (0,-4.5) {GP Model\\$\mu_g(x,u),\;\sigma_g^2(x,u)$};
		\node[block] (nom) at (6,-4.5) {Nominal Model\\$Ax_k + Bu_k$};
		\node[sum] (sub) at (9,-2.5) {$\boldsymbol{-}$};

		% ---- 1. MPC -> Plant ----
		\draw[arr] (mpc.east) -- node[above] {$u_k^*$} (plant.west);

		% ---- 2. Plant -> x_{k+1} junction ----
		\coordinate (xjunc) at (9, 0);
		\draw[thick] (plant.east) -- (xjunc);
		\node[above, font=\small] at (xjunc) {$x_{k+1}$};

		% ---- 3. Feedback: x_k to MPC ----
		\draw[arr] (xjunc) -- ++(0.8,0) |- ([yshift=1.0cm]plant.north)
			-| node[above, pos=0.25, font=\small] {$x_k$ (measured)} (mpc.north);

		% ---- 4. x_{k+1} down to subtraction (+ input) ----
		\draw[arr] (xjunc) -- node[right, font=\scriptsize, pos=0.7] {$+$} (sub.north);

		% ---- 5. Nominal model -> subtraction (- input) ----
		\draw[arr] (nom.east) -- node[below, font=\small] {$Ax_k\!+\!Bu_k$}
			(nom.east -| sub) -- node[right, font=\scriptsize, pos=0.8] {$-$} (sub.south);

		% ---- 6. Subtraction output: right -> down -> left -> up into GP.south ----
		\draw[arr] (sub.east) -- ++(1.0,0) |- ([yshift=-0.8cm]gp.south)
			-- (gp.south);
		\node[below, font=\small] at ([yshift=-0.8cm]gp.south) {$r_k$};

		% ---- 7. GP -> MPC ----
		\draw[arr] (gp.north) -- node[left, font=\small] {$\mu_g,\;\sigma_g^2$} (mpc.south);

		% ---- 8. x_k, u_k to Nominal Model ----
		\draw[arr, gray, dashed] ([yshift=-0.5cm]plant.south) -- node[right, font=\scriptsize, text=gray] {$x_k,\,u_k$} (nom.north);
	\end{tikzpicture}
...
We then seek a matrix $K_d \in \mathbb{R}^{p \times p}$ (we write $K_d$ to distinguish it from the feedback gain $K$ used elsewhere in this book) such that:
...
% Terminal cost via DARE (as in Chapter 7)
[~, P_lift] = dlqr(A_K, B_K, Q_lift, R);   % lifted system
[~, P_lin]  = dlqr(A_lin, B_lin, Q, R);     % linearised system
...
for k = 1:N
    xk = Cx*zp{k+1};
    objK = objK + R*uK(k)^2 + lam_s*sK(k)^2;
    if k < N
        objK = objK + xk'*Q*xk;             % stage cost
    else
        objK = objK + zp{k+1}'*P_lift*zp{k+1}; % terminal cost
    end
...
objL = 0;  conL = [sL >= 0];
for k = 1:N
    objL = objL + R*uL(k)^2 + lam_s*sL(k)^2;
    if k < N
        objL = objL + xp{k+1}'*Q*xp{k+1};     % stage cost
    else
        objL = objL + xp{k+1}'*P_lin*xp{k+1};  % terminal cost
    end
...
\begin{examplebox}{DeePC for a Scalar Integrator}
	...
	\emph{Data.} $T = 8$ samples with PE input $u^d = [1,\; -1,\; 0.5,\; -0.5,\; 0.8,\; -0.2,\; 0.3,\; -0.9]$, starting from $y_0^d = 0$:
	\[
	y^d = [0,\; 1,\; 0,\; 0.5,\; 0,\; 0.8,\; 0.6,\; 0.9]
	\]
	...
	\begin{align*}
		U_p &= \begin{bmatrix} 1 & -1 & 0.5 & -0.5 & 0.8 & -0.2 \end{bmatrix} \\
		U_f &= \begin{bmatrix} -1 & 0.5 & -0.5 & 0.8 & -0.2 & 0.3 \\ 0.5 & -0.5 & 0.8 & -0.2 & 0.3 & -0.9 \end{bmatrix}
	\end{align*}
...
\begin{equation}\label{eq:cost_to_go}
	Q^j(x) = \min_{(i,k):\, x_k^{(i)} = x}\; \sum_{t=k}^{T_i-1} \ell\bigl(x_t^{(i)},\, u_t^{(i)}\bigr)
\end{equation}
...
LMPC provides two important properties without requiring offline design of terminal ingredients (the dynamics model $f$ is still needed for prediction):
...
\begin{examplebox}{LMPC: Scalar Minimum-Time Problem}
	...
	\emph{Iteration 0: conservative controller.} Use $u_k = -0.3$ at most steps; near the origin ($|x_k| \le 0.3$), switch to $u_k = -x_k$ to reach $x_F = 0$ exactly.
	\[
	\begin{array}{c|c c c c c c c c}
		k & 0 & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\
		\hline
		x_k^{(0)} & 2.0 & 1.7 & 1.4 & 1.1 & 0.8 & 0.5 & 0.2 & 0.0
	\end{array}
	\]
	Cost: $J^{(0)} = 7$ steps.
...
The track centreline is taken from the MPCC benchmark~\cite{liniger2015optimization}, a complex racing circuit of approximately 445\,m featuring tight hairpins, chicanes, and high-speed straights. The centreline is parameterised by arc length $s \in [0, L)$, where $L$ is the total track length. At each point, the unit tangent $\mathbf{t}(s)$ and outward normal $\mathbf{n}(s)$ are computed from the spline derivatives, and the local curvature $\kappa(s)$ is obtained from the rate of change of the tangent angle:
\begin{equation*}
...
\begin{enumerate}
	\item A \emph{speed map} $v_\theta(s)$ that predicts the target speed at each track position~$s$, replacing the safe-set--based speed targets.
	\item A \emph{value function} $V_\phi(\mathbf{x})$ that approximates the cost-to-go from any state, replacing the convex-combination terminal cost $Q^j(x_N)$.
\end{enumerate}
...
\end{equation*}
where $T_i$ is the total number of steps in iteration $i$. Training data from all completed laps is accumulated; for states visited in multiple iterations, only the minimum cost-to-go is retained, consistent with~\eqref{eq:cost_to_go}. The network is retrained after each iteration. The input features are:
\begin{equation*}
...
\paragraph{Step~2: Lookahead braking.} The controller checks the speed map at lookahead times $\Delta t_{\mathrm{look}} \in \{0.5,\, 1.0,\, 1.5\}$\,s ahead. If the target speed is lower ahead (a corner is approaching), the current target is reduced to allow deceleration:
...
\For{$j = 1$ \textbf{to} $J-1$}
    ...
    \State Apply boost $\tilde{v}_b^{(j)} = \bar{v}_b^{(j)} \cdot \beta_j$, cap by curvature $v_{\max}(s)$ \hfill \Comment{Eqs.~\eqref{eq:boost},~\eqref{eq:curv_limit}}
...
\begin{enumerate}
	...
	\item \emph{Off-track abort.} If the lateral deviation from the centreline exceeds $3w$ (15\,m), the lap is aborted. In practice, mechanisms~1--3 keep the deviation within the track boundaries. Early iterations stay close to the centreline ($\approx 2$\,m), while later, faster iterations exploit the full track width ($\approx 5$\,m)---mirroring real racing where drivers use the entire road.
\end{enumerate}


\subsubsection{Safe Set Expansion Over Iterations}

Figure~\ref{fig:nnmpc_safe_set} shows how the safe set grows over iterations. After iteration~0, only a narrow band of states near the centreline has been explored (at 5\,m/s). By iteration~7, the safe set contains over 10\,000 state measurements spanning a wide range of speeds and positions. The colour encodes longitudinal speed---the progression from blue (slow) to red (fast) shows that later iterations reach higher speeds on the straights whilst maintaining safe speeds through corners.
...
The simulation runs 8~iterations on the MPCC benchmark track (445\,m). Total computation time is approximately 15\,s on a standard laptop. Figure~\ref{fig:nnmpc_racing_results} presents the results in four panels.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.95\textwidth]{Figures/fig_nnmpc_racing_results.pdf}
	\caption{NN-MPC autonomous racing results over 8~iterations on the MPCC benchmark track. Top left: track with overlaid trajectories---later iterations carry higher speed through straights whilst maintaining the racing line through corners. Top right: lap times decrease monotonically from 90.3\,s (pure pursuit at 5\,m/s) to 56.3\,s (NN-MPC at average 7.8\,m/s), a 38\% improvement. Bottom left: actual speed profiles---the car learns to accelerate harder on straights and brake later into corners. Bottom right: NN speed map evolution showing how the learned target speeds increase with each iteration (capped by curvature at corners).}
	\label{fig:nnmpc_racing_results}
\end{figure}

The key observations are:
\begin{itemize}
	\item \emph{Monotone cost decrease.} Lap times decrease from 90.3\,s to 56.3\,s (38\% improvement), consistent with the non-increasing cost property of LMPC (Section~\ref{sec:lmpc_guarantees}).

	\item \emph{Speed profiles.} Iteration~0 drives at a near-constant 5\,m/s. By iteration~7, the car reaches approximately 8\,m/s on average and up to 12\,m/s on the straights, braking to approximately 6\,m/s through tight corners---the curvature speed limit~\eqref{eq:curv_limit} is the binding constraint.
	...
	\item \emph{Track usage.} Early iterations stay near the centreline (max lateral deviation $\approx 2$\,m). As speeds increase, the car exploits the full track width---reaching up to 5\,m from the centreline in later iterations. This mirrors real racing behaviour where drivers use the entire road to maximise cornering speed. The vehicle stays within the 5\,m half-width throughout all iterations.
\end{itemize}
...
\begin{table}[ht]
	\centering
	\caption{NN-MPC racing results: lap times and average speeds over 8~iterations on the 445\,m MPCC benchmark track.}
	\label{tab:racing_results}
	\begin{tabular}{crrrc}
		\toprule
		\textbf{Iteration} & \textbf{Lap time [s]} & \textbf{Avg.\ speed [m/s]} & \textbf{Max $|e_d|$ [m]} & \textbf{Controller} \\
		\midrule
		0 & 90.3 & 4.9 & 2.2 & Pure pursuit \\
		1 & 80.3 & 5.6 & 2.2 & NN-MPC \\
		2 & 73.7 & 6.1 & 2.7 & NN-MPC \\
		3 & 68.6 & 6.5 & 2.4 & NN-MPC \\
		4 & 65.1 & 6.9 & 3.6 & NN-MPC \\
		5 & 61.8 & 7.3 & 5.1 & NN-MPC \\
		6 & 58.0 & 7.6 & 4.2 & NN-MPC \\
		7 & 56.3 & 7.8 & 4.3 & NN-MPC \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}
	\item \emph{Linearise the NN} at each time step (similar to the Real-Time Iteration scheme in Chapter~\ref{chap:nmpc}), producing a time-varying QP.
