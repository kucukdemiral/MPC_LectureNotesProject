...
\begin{quote}
	...
	\hfill --- paraphrasing \emph{David Q.\ Mayne}
\end{quote}
...
\begin{enumerate}
	\item A \emph{nominal trajectory} $z_k$ that is planned as if there were no disturbance. This is a standard MPC problem.
	\item A \emph{feedback controller} $-K(x_k - z_k)$ that keeps the real state $x_k$ close to the nominal trajectory $z_k$, despite the disturbance.
\end{enumerate}

\begin{notebox}{Notation: $z_k$ in This Chapter}
	Throughout this chapter, $z_k$ denotes the \emph{nominal state} (the disturbance-free planned trajectory). This is unrelated to the augmented state vector $z_k = [x_k^\top\;\; d_k^\top]^\top$ used in the offset-free MPC formulation of Chapter~\ref{chap:output_fb_mpc}. The two uses of $z_k$ never appear in the same context.
\end{notebox}
...
This is the essence of Tube MPC: \emph{plan the nominal path conservatively, and let a feedback controller handle the disturbances.}
...
where:
\begin{itemize}[noitemsep]
	\item $v_k$ is the \emph{nominal input}, computed by the MPC optimiser. It steers the nominal trajectory $z_k$ toward the origin.
	\item $K$ is a \emph{fixed feedback gain} (typically the LQR gain from \texttt{dlqr}, chosen so that $A - BK$ is Schur stable). It keeps the real state close to the nominal state by rejecting disturbances.
\end{itemize}
...
The smallest such set is the \emph{minimal RPI (mRPI) set}, denoted $\mathcal{E}_\infty$~\cite{Rakovic2005}. It can be characterised explicitly:
...
\begin{center}
	\begin{tikzpicture}[
		...
		greenblock/.style={block, draw=teal, fill=teal!8},
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\linewidth, trim=0 0 0 22, clip]{Figures/tube_mpc_concept.pdf}
