...
While the concept of \emph{Observability} tells us \textit{if} the state can be reconstructed mathematically, it does not tell us \textit{how} to do so robustly in the presence of uncertainty. This chapter derives the optimal solution to the state estimation problem: the \emph{Kalman Filter}~\cite{Kalman1960}.
...
In many practical control scenarios, we cannot know the state of a system with absolute certainty due to sensor noise, model mismatch, or unknown disturbances. Consequently, we model the state not as a deterministic scalar or vector, but as a \emph{random vector} $x \in \mathbb{R}^n$. This means $x$ does not hold a single fixed value, but is described by a probability distribution function (PDF), denoted as $p(x)$.

The most fundamental statistical property of a random variable is the \emph{Mean} (or Expected Value). Intuitively, this represents the ``centre of gravity'' of the probability distribution, or the average value one would observe if the experiment were repeated an infinite number of times. Mathematically, it is the first moment of the distribution:
\begin{equation*}
	\mu = \mathbb{E}[x] = \int_{-\infty}^{\infty} x \, p(x) \, dx
\end{equation*}
Note that for a vector $x$, this integral is applied element-wise to each component.

A crucial property of the expectation operator $\mathbb{E}[\cdot]$ is that it is \emph{linear}. If we apply a deterministic linear transformation to a random variable—such as multiplying it by a constant matrix $A$ or shifting it by a constant vector $b$—the mean transforms in exactly the same manner:
\begin{equation*}
...
While the mean tells us where the state is \textit{likely} to be, it tells us nothing about how confident we are in that estimate. This uncertainty is quantified by the \emph{Covariance Matrix}, typically denoted as $P$ (or $\Sigma$).
...
In linear estimation theory, we predominantly assume that noise and states follow a \emph{Gaussian} (or Normal) distribution (see Appendix~\ref{app:prob} for a detailed treatment of probability distributions). A Gaussian random vector is completely defined by its mean $\mu$ and covariance $P$. We write this as:
\begin{equation*}
...
In system modelling, we often represent disturbances as \emph{White Noise}. A discrete-time stochastic process $w_k$ is called white noise if it is uncorrelated in time:
\begin{equation*}
	\begin{aligned}
		\mathbb{E}[w_k] &= 0 \\
	...
	\end{aligned}
\end{equation*}
Here, $W_k$ is the \emph{Process Noise Covariance}. Similarly, for sensors, we define measurement noise $v_k$ with \emph{Measurement Noise Covariance} $V_k$.
...
We define the \emph{estimation error} as:
\begin{equation*}
	e_k \triangleq x_k - \hat{x}_k
\end{equation*}

The problem is to find the recursive algorithm that computes $\hat{x}_k$ such that the expected value of the squared error norm (the \emph{Mean Squared Error}) is minimised at every time step:
\begin{equation*}
	\min_{\hat{x}_k} J_k = \mathbb{E} \left[ \| e_k \|^2 \right] = \mathbb{E} \left[ (x_k - \hat{x}_k)^\top (x_k - \hat{x}_k) \right]
\end{equation*}

An estimator that satisfies this condition is known as the \emph{Minimum Variance Unbiased Estimator (MVUE)}. Under the Gaussian assumptions listed above, the Kalman Filter is the unique solution to this problem.


\section{Derivation of the Kalman Filter}

To derive the optimal filter, we postulate a recursive linear estimator structure that operates in two distinct phases at each time step: a \emph{Prediction} phase (based on the system model) and a \emph{Correction} phase (based on the new measurement).
...
\begin{enumerate}
	...
	The prediction is corrected using the difference between the actual measurement and the predicted measurement. This difference is called the \emph{Innovation} ($\tilde{y}_k$):
	\begin{equation} \label{eq:obs_correct}
		\hat{x}_k = \hat{x}_k^- + L_k \underbrace{(y_k - C \hat{x}_k^-)}_{\tilde{y}_k}.
	\end{equation}
\end{enumerate}

Here, $L_k$ is the \emph{Kalman gain}. Our objective is to determine $L_k$ such that the mean squared estimation error is minimised.
...
\end{itemize}
Combining these results yields the \emph{Joseph form} covariance update:
...
\end{equation*}
The term in the parentheses represents the uncertainty of the measurement prediction. We define this as the \emph{Innovation Covariance}, $S_k$:
\begin{equation*}
...
However, for \emph{Linear Time-Invariant (LTI)} systems—where $A, B, C, W, V$ are constant—the covariance matrix $P_k^-$ converges to a constant steady-state matrix $P_{\infty}$ as $k \to \infty$, provided certain structural properties are met.
...
A remarkable result in linear systems theory is the mathematical symmetry, or \emph{duality}, between the optimal control problem (LQR) and the optimal estimation problem (Kalman Filter).
...
Despite the difference in the direction of time, the recursive equations governing these two matrices are identical under a specific substitution of variables. This means that the \emph{same numerical algorithms} (such as the Riccati solver) used to design a controller can be used to design an observer.
...
While the system matrices ($A, B, C$) obey physical laws, the noise covariance matrices often serve as tuning parameters that dictate the filter's \emph{behaviour}.
...
The matrix $W$ is more abstract. It lumps together physical disturbances (e.g., wind gusts) and \emph{modelling errors} (e.g., linearisation inaccuracies). Since these are difficult to quantify, $W$ is treated as a tuning handle to adjust the trade-off between the model and the measurements:
...
\begin{examplebox}{MATLAB: Kalman Filter Implementation (LQG Control)}
	\textbf{The Problem:}
	Consider the double integrator system (representing a simple mass). Our objective is to stabilise the system using full-state feedback ($u_k = -K x_k$). However, we face a practical constraint: we can only measure the \emph{position}, and that measurement is corrupted by noise. We cannot measure the velocity directly.
	
	\textbf{The Solution:}
	We design a \emph{Linear Quadratic Gaussian (LQG)} controller. This consists of two parts:
	\begin{enumerate}
...
\begin{itemize}
    \item The state estimation problem seeks to reconstruct unmeasured or noisy states from available output measurements and a known system model.
    \item The Kalman filter operates in two recursive steps: a \emph{prediction} step that propagates the state and covariance forward, and an \emph{update} step that corrects them using new measurements.
    \item The Kalman gain is derived by minimising the trace of the posterior error covariance, yielding an optimal linear estimator for Gaussian noise.
    \item Covariance propagation equations track the uncertainty of the state estimate through both the prediction and update phases.
    \item The steady-state Kalman filter replaces the time-varying gain with a constant gain obtained by solving the discrete algebraic Riccati equation, reducing online computation.
    \item A formal duality exists between the Kalman filter and the LQR problem: the estimator Riccati equation mirrors the controller Riccati equation with transposed system matrices and swapped noise/cost weighting.
    \item The Linear-Quadratic-Gaussian (LQG) controller combines LQR state feedback with a Kalman filter via the separation principle, enabling optimal output-feedback control.
    \item Tuning the Kalman filter involves adjusting the process noise covariance $Q$ and measurement noise covariance $R$ to balance responsiveness against sensitivity to noise.
\end{itemize}
