...
With the state-space modelling framework from Chapter~\ref{chap:statespace} in hand, we address two structural questions that determine whether a control or estimation design is even possible.

Earlier chapters developed the tools of optimisation and state-space modelling. Before asking ``what is the optimal trajectory?'', we must answer a more basic question: ``does a valid trajectory exist?''

This chapter introduces the twin structural properties of \emph{controllability} and \emph{observability}. These concepts define the physical limitations of what a control system can achieve, regardless of the sophistication of the algorithm used.
...
While this matrix grows with the horizon $N$, the \emph{Cayley-Hamilton Theorem} tells us that any matrix power $A^k$ for $k \ge n$ can be written as a linear combination of lower powers ($I, \dots, A^{n-1}$). Consequently, adding columns beyond $A^{n-1}B$ provides no new linearly independent directions.
...
We can therefore define the standard \emph{Controllability Matrix} $\mathcal{C}$ using only $n$ blocks.
...
\begin{alertbox}{Theorem: The Kalman Rank Condition}
	Consider an LTI system with state dimension $n$. The system pair $(A, B)$ is controllable if and only if the \emph{controllability matrix} $\mathcal{C}$ has full rank:
	\begin{equation*}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[auto, >=latex', thick]
		% Define styles
		\tikzset{block/.style={draw, rectangle, fill=gray!20, minimum height=3em, minimum width=6em, align=center}}
		\tikzset{dashedblock/.style={draw, dashed, rectangle, fill=red!8, minimum height=3em, minimum width=6em, align=center}}
		\tikzset{line/.style={draw, ->}}
		\tikzset{bigbox/.style={draw, dashed, rounded corners, inner sep=10pt}}
		...
		\draw [line, dashed, red!70!black] (-0.3, -0.3) -- (-0.3, -3) -- (controllable |- uncontrollable.west);
		\node [red!70!black, font=\Large\bfseries] at (-0.3, -1.5) {$\times$};
		\node [red!70!black, font=\small, left] at (-0.55, -1.5) {No input authority};
...
While controllability deals with the active problem of steering the state $x$ using the input $u$, \emph{observability} addresses the passive inverse problem: can we reconstruct the internal state $x(t)$ solely by monitoring the system's input $u(t)$ and output $y(t)$?
...
\begin{defnbox}{Observability}
	A linear system is defined as \emph{observable} if the initial state $x(t_0)$ can be uniquely determined from the knowledge of the system input $u(t)$ and output $y(t)$ over a finite time interval $t_0 \leq t \leq t_1$. 
	
	If a system is \emph{unobservable}, there exists at least one "hidden" mode inside the system that does not affect the output $y(t)$. Consequently, the controller acts "blindly" with respect to this mode, which can lead to undetected instability.
\end{defnbox}
...
To test for this property, we construct the \emph{Observability Matrix}, denoted by $\mathcal{O}$:
\begin{equation*}
	\mathcal{O} = 
	\begin{bmatrix}
		C \\
...
\begin{notebox}{Kalman Rank Condition for Observability}
	An LTI system is fully observable if and only if the observability matrix has \emph{full column rank}:
	\begin{equation*}
...
\begin{itemize}
	...
	\item $\mathcal{O}$ is the \emph{Observability Matrix} (known).
	\item $x_0$ is the state vector we wish to find (unknown).
\end{itemize}

\subsubsection*{Step 4: The Invertibility Condition}
This is a standard linear algebra problem of the form $Ax = b$. We have $n$ unknowns (the elements of $x_0$). To find a \emph{unique} solution for $x_0$, the matrix $\mathcal{O}$ must provide enough independent equations to constrain the solution fully.
...
\textit{Note:} One might ask, "Why stop at $n-1$?" According to the \emph{Cayley-Hamilton Theorem}, any matrix power $A^k$ for $k \ge n$ can be written as a linear combination of lower powers ($I, A, \dots, A^{n-1}$). Therefore, taking more measurements beyond $k=n-1$ adds rows to $\mathcal{O}$ that are linearly dependent on previous rows, providing no new information for determining the rank.
...
A pair $(A, B)$ is \emph{stabilisable} if every uncontrollable mode of $A$ is stable (i.e.\ has eigenvalue magnitude less than 1 in discrete time, or negative real part in continuous time). Equivalently, $(A, B)$ is stabilisable if there exists a feedback gain $K$ such that $A - BK$ is Schur stable.

A pair $(A, C)$ is \emph{detectable} if every unobservable mode of $A$ is stable. Equivalently, $(A, C)$ is detectable if there exists an observer gain $L$ such that $A - LC$ is Schur stable.
\end{defnbox}
...
This chapter established the structural properties of linear systems that determine whether MPC can steer and observe the plant. The key topics covered are:

\begin{itemize}
	\item \textbf{Controllability:} A system $(A, B)$ is controllable if the state can be driven from any initial condition to any target in finite time. The Kalman rank condition ($\operatorname{rank}[\,B\;\; AB\;\; \cdots\;\; A^{n-1}B\,] = n$) provides a direct algebraic test.

	\item \textbf{PBH Test:} The Popov--Belevitch--Hautus eigenvector test offers an alternative: $(A, B)$ is controllable if and only if $\operatorname{rank}[\,\lambda I - A\;\; B\,] = n$ for every eigenvalue $\lambda$ of $A$. This test reveals which modes are uncontrollable.

	\item \textbf{Observability:} The dual property to controllability, observability ensures that the full state can be reconstructed from output measurements. The observability matrix $\mathcal{O}$ must have full column rank.

	\item \textbf{Stabilisability and Detectability:} These are the relaxed versions of controllability and observability, requiring only that the \emph{unstable} modes be controllable or observable. They are the minimal conditions needed for MPC feasibility and observer convergence.

	\item \textbf{Kalman Decomposition:} A state-space transformation decomposes the system into controllable/uncontrollable and observable/unobservable subsystems, providing structural insight into which parts of the dynamics can be influenced and measured.

	\item \textbf{Implications for MPC:} Stabilisability of $(A, B)$ is necessary for the existence of a stabilising MPC law (via the DARE solution), while detectability of $(A, C)$ is required for state estimation. Lack of these properties leads to infeasibility or unbounded costs.
\end{itemize}
...
\item The \emph{Popov--Belevitch--Hautus (PBH) test} provides an eigenvalue-based criterion for controllability: the pair $(A,\,B)$ is controllable if and only if
\[
