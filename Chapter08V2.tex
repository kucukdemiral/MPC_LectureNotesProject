...
To handle this, we reformulate the target calculation as a constrained \emph{steady-state optimisation} (SSO) problem. Instead of insisting on exact equality \(y_{ss}=r_k\), we minimise the tracking error whilst enforcing the equilibrium equations and any relevant constraints.
...
regulation layer is designed in \emph{deviation variables}:
\[
...
cancel exactly, leaving the \emph{disturbance-free linear deviation
	dynamics}:
\[
\tilde x_{i+1} = A \tilde x_i + B \tilde u_i.
\]
...
\begin{examplebox}{MATLAB: Offset-Free MPC for Quadrotor Position Control}
	...
	We now define the \emph{virtual control inputs}
	\[
...
\begin{itemize}
    \item Output feedback MPC replaces full-state measurements with observer-based state estimates in the MPC optimisation loop.
    \item The separation principle, which guarantees independent design of estimator and controller in LQG, does not extend directly to constrained MPC and requires careful treatment.
    \item A disturbance model augments the plant state with an integrating disturbance, enabling the estimator to detect and compensate for persistent model--plant mismatch.
    \item Offset-free MPC uses the augmented observer's disturbance estimate in a steady-state target calculator to eliminate steady-state tracking errors.
    \item The steady-state target calculator solves a small optimisation problem at each step to find the closest feasible equilibrium consistent with the estimated disturbance.
    \item The observer-based MPC architecture combines an augmented Kalman filter, a steady-state target calculator, and a dynamic MPC optimiser into a modular feedback loop.
    \item Practical examples, including quadrotor altitude control, building HVAC regulation, and robot arm trajectory tracking, illustrate the design methodology on systems with different constraint structures.
\end{itemize}
